...
namespace Integration.Infrastructure.Repositories;
    ...
    public class AuthRepository : IAuthRepository
    {
            ...
            try
            {
                return await _context.Users.AsNoTracking()
                    .FirstOrDefaultAsync(u =>
                        u.BusinessUnit == businessUnit &&
                        u.UserName == username &&
                        u.Status == "1");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to get user {Username} in BU {BusinessUnit}",
        ...
        public async Task CreateUserSessionAsync(UserSession session)
        {
            ...
            {
                _logger.LogError(ex, "Failed to create user session for user {UserId}",
                    session.UserID);
                throw;
            }
        }
        ...
        public async Task UpdateUserSessionAsync(UserSession session)
        {
            ...
            {
                _logger.LogError(ex, "Failed to update user session {SessionId}",
                    session.RecID);
                throw;
            }
        }
        ...
        public async Task<bool> CreateUserAsync(User user)
        {
            try
            {
                var existing = await _context.Users
                    .AnyAsync(u => u.BusinessUnit == user.BusinessUnit &&
        ...
        public async Task<int> GetActiveSessionsCountAsync(long userId)
        {
            ...
            {
                return await _context.UserSessions
                    .CountAsync(s => s.UserID == userId && s.Status == "1" && s.ExpiresAt > DateTime.Now);
            }
            catch (Exception ex)
            {
